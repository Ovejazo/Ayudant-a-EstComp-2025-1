...
x <- seq(media - 4*varianza, media + 4*varianza, length.out = 1000)

# Distribución normal para Y (densidad de probabilidad)
y <- dnorm(x, mean = media, sd = varianza)
...
ggplot(df, aes(x = x, y = y)) +
  ...
  geom_ribbon(data = df[x >= (media - varianza) & x <= (media + varianza), ],
...
x=seq(1,7,by=0.01) # Creacion de arreglo para eje x
y=dnorm(x,prom,varianza) # Distribucion normal

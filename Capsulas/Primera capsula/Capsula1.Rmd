...
---
Ya tengo la idea clara al menos.
Por lo que puedo ver, el profe no paso TODAS las distribuciones, por lo cual solo me enfocare en hacer ejercicios de distribución normal, de Poisson y binomial, que son las que el profe paso en clases, de todas formas me gustaría pasar algunas distribuciones más (Como Bernoulli).
...
```{r}
normal_percentil = qnorm(0.95, mu = mu, sd = sigma)
```
...
La fórmula de la distribución de Poisson es \( P(X = k) = \frac{\lambda^k e^{-\lambda}}{k!} \)
...
```{r}
poisson_acumulado_mas = 1 - dpois(q = 2, lambda = 4)
